...
;Miller-Rabin test (Miller 1976; Rabin 1980). This starts from an alternate form
;of Fermat’s Little Theorem, which states that if n is a prime number and a is
...
;the numbers a < n, computing a^{n−1} in this way will reveal a nontrivial
...
(check-true (fast-prime? 13 10))
(check-true (fast-prime? 839 10))
(check-true (fast-prime? 1009 10))
...
(check-true (not (fast-prime? 529 10)))

;Carmichael numbers
(check-true (not (fast-prime? 561 10)))
(check-true (not (fast-prime? 1105 10)))
(check-true (not (fast-prime? 1729 10)))
(check-true (not (fast-prime? 2465 10)))
(check-true (not (fast-prime? 2821 10)))
(check-true (not (fast-prime? 6601 10)))